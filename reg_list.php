<?php
    ...
    if ($form == "DisplayList" || isset($_POST['DownloadList'])) {
        ...
        echo "<script type='text/javascript'>
            window.open('../downloadExcel.php?level=$level&type=$type&examID=$examID', '_blank')
        </script>";        
    }
    ...
    if ($form == "DisplayList" || isset($_POST['DisplayList'])) {
        ...
        displayList($type, $level, $examID, $userDataResult, $courseColumns);
    }
?>
    ...
    <div class="w-11/12 mb-10 mx-auto flex flex-col gap-y-5 text-center">
        ...
        <form action="" method="post" class="w-full grid grid-cols-3 gap-x-5 items-center content-center">
            ...
            <select class="!w-11/12 border-2 rounded-full py-2 px-5" id="type" name="type"  required>
                <option value="select" <?php setSelected('type', 'select') ?> disabled selected>Select Type <span class="!text-red-500">*</span></option>
            ...
            <select class="!w-11/12 border-2 rounded-full py-2 px-5" id="level" name="level" required>
                <option value="select" <?php setSelected('level', 'select') ?> disabled selected>Select Level <span class="!text-red-500">*</span></option>
...
<?php function displayList($type, $level, $examID, $userDataResult, $courseColumns) {
    global $con; ?>
    
    <div class="flex flex-col gap-y-1 items-center">
    <?php if (isset($_SESSION['role'])) { ?>
        <a href="<?php echo "../downloadExcel.php?level=$level&type=$type&examID=$examID" ?>" target="_blank" class="btn outline-btn !text-green-600 !border-green-500 !bg-white hover:!bg-green-500 hover:!text-white">Download as a Excel sheet</a>
